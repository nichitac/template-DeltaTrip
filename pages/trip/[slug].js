...
export default function PostPage({
  slug,
  content,
  frontMatter: { title, boat, date, image, description, tags },
  boats,
}) {
  let pageUrl = `${siteConfig.baseURL.replace(/\/$|$/, "/")}trip/${slug}`;
  return (
    <Layout metaTitle={title} metaDescription={description} ogImage={image}>
      <section className="section-sm pb-0">
        <div className="container">
          ...
          <div className="single-post-boat">
            <div className="row justify-content-center">
              <div className="col-lg-10">
                <div className="d-block d-md-flex">
                  <Link
                    href={`/boat/${boat.replace(/ /g, "-").toLowerCase()}`}
                  >
                    <a>
                      {boats.map((boatPage, i) =>
                        boat.replace(/ /g, "-").toLowerCase() ===
                        boatPage.boatSlug ? (
                          <span key={i}>
                            <Image
                              ...
                              placeholder="blur"
                              blurDataURL={boatPage.boatFrontMatter.image}
                            />
                          </span>
                        ) : (
                          ""
                      ...
                      boatPage.boatSlug ? (
                        <div
                          ...
                          dangerouslySetInnerHTML={{
                            __html: marked.parse(
                              truncateString(boatPage.boatContent, 150)
                            ),
                          }}
                        ></div>
                      ) : (
...
export async function getStaticPaths() {
  ...
  const paths = trips.map((filename) => ({
    params: {
...
export async function getStaticProps({ params: { slug } }) {
  const fileContents = fs.readFileSync(
    path.join("content/trip", slug + ".md"),
    "utf8"
  );
